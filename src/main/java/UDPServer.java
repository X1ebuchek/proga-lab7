...
//                }
                ...
                Runnable task = () -> listen(socketAddress1,channel,bytebufferToReceive, finalSerializationHelper, finalDeserializationHelper, receiver2);
                service.submit(task);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void listen(SocketAddress socketAddress1, DatagramChannel channel, ByteBuffer bytebufferToReceive, SerializationHelper serializationHelper, DeserializationHelper deserializationHelper, Receiver2 receiver2){
        try {
            socketAddress1 = channel.receive(bytebufferToReceive);
            if (socketAddress1 != null) {
                logger.info("The server received a request");
                SocketAddress finalSocketAddress = socketAddress1;
                Runnable task = () -> execution(finalSocketAddress,channel,bytebufferToReceive, serializationHelper, deserializationHelper, receiver2);
                service.submit(task);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void execution(SocketAddress socketAddress1, DatagramChannel channel, ByteBuffer bytebufferToReceive, SerializationHelper serializationHelper, DeserializationHelper deserializationHelper, Receiver2 receiver2){
        sendThing = (SendThing) deserializationHelper.deSerialization(bytebufferToReceive);
        String k = receiver2.receive(sendThing.getCommand(),sendThing.getTicket(),sendThing.getArgument(),sendThing.getLogin());
        bytebufferToReceive.clear();
        Runnable task = () -> sending(socketAddress1,k,serializationHelper,channel);
        Thread thread = new Thread(task);
        thread.start();
    }
    public void sending(SocketAddress socketAddress1,String k, SerializationHelper serializationHelper, DatagramChannel channel){
        Answer answer = new Answer(k);
        bytebufferToSend = serializationHelper.serialize(answer);
        try {
            channel.send(bytebufferToSend, socketAddress1);
